...
function ENT:Think()
    if CLIENT then
        ...
        local target = (self.DataForProjectiles or {}).FF_Target
        ...
        local phys = self:GetPhysicsObject()
        phys:ApplyForceCenter( self:GetAngles():Forward() * 35000 )
...
function ENT:Detonate()
    ...
    if self:WaterLevel() >= 1 then
        ...
        self:EmitSound("weapons/underwater_explode3.wav", 130, 110, 1, CHAN_AUTO)
    else
        util.Effect( "act3_rocketexplosion", effectdata)
        self:EmitSound("phx/explode00.wav", 130, 110, 1, CHAN_AUTO)
    end
    ...
    self:FireBullets({
        ...
        Callback = function(att, tr, dmg)
            util.Decal("SmallScorch", tr.StartPos, tr.HitPos - (tr.HitNormal * 16), self)
